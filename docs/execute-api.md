...
| `token_usage`     | object | Token counts consumed by this call. Absent if the provider did not report usage.  |
| `trace`           | array  | LLM round-trip trace. Only present when the account has `traces_read` permission. |

### `token_usage` fields

When tools are used, the model is called more than once (`llm_calls > 1`). `token_usage` reports the **sum across all rounds**, so it reflects the full cost of the request.

| Field               | Type | Description                                                                         |
| ------------------- | ---- | ----------------------------------------------------------------------------------- |
| `prompt_tokens`     | int  | Input tokens sent to the model (total across all LLM rounds).                       |
| `completion_tokens` | int  | Output tokens generated by the model (total across all LLM rounds).                 |
| `total_tokens`      | int  | `prompt_tokens + completion_tokens`. Computed locally if the provider omits it.     |
| `reasoning_tokens`  | int  | Tokens spent on chain-of-thought reasoning (subset of `completion_tokens`). Omitted when zero. |
| `cached_tokens`     | int  | Prompt tokens served from the provider's cache (subset of `prompt_tokens`). Omitted when zero. |
