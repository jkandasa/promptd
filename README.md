...
`promptd` is a Go-based prompt server with an embedded React UI for multi-provider chat, RBAC, MCP tool calling, persistent conversation history, conversation compaction, and scheduled prompt execution.

## Current Scope

- OpenAI-compatible multi-provider backend
- YAML-backed persistence scoped by tenant and user
- Browser login with JWT cookie auth plus bearer service tokens
- Role-based access control for chat, uploads, conversations, schedules, traces, compaction, models, tools, and system prompts
- Embedded React app for chat, scheduler, and tool visibility
- MCP server integration with reconnect, health checks, and tool rediscovery
- Rolling conversation compaction with auto and manual modes
- Scheduler with cron and one-time jobs, execution history, and trace visibility
- Optional HTTPS with provided certs or autogenerated self-signed certs
...
- Detailed LLM trace capture and UI trace drawers
