...
func RegisterRoutes(mux *http.ServeMux, authService *auth.Service, h *handler.Handler, mcpHandler *handler.MCPToolsHandler, schedHandler *handler.ScheduleHandler) {
	...
	mux.Handle("POST /api/auth/change-password", requireAuth(http.HandlerFunc(h.ChangePassword)))
	mux.Handle("GET /api/admin/auth", requireAuth(http.HandlerFunc(h.ListAuthConfig)))
	mux.Handle("POST /api/admin/users", requireAuth(http.HandlerFunc(h.SaveUser)))
	mux.Handle("DELETE /api/admin/users/{id}", requireAuth(http.HandlerFunc(h.DeleteUser)))
	mux.Handle("POST /api/admin/users/{id}/api-keys", requireAuth(http.HandlerFunc(h.GenerateAPIKey)))
	mux.Handle("DELETE /api/admin/users/{id}/api-keys/{keyId}", requireAuth(http.HandlerFunc(h.DeleteAPIKey)))
	mux.Handle("PATCH /api/admin/users/{id}/api-keys/{keyId}", requireAuth(http.HandlerFunc(h.UpdateAPIKey)))
	mux.Handle("POST /api/admin/roles", requireAuth(http.HandlerFunc(h.SaveRole)))
	mux.Handle("DELETE /api/admin/roles/{name}", requireAuth(http.HandlerFunc(h.DeleteRole)))
	...
	mux.Handle("GET /api/user/api-keys", requireAuth(http.HandlerFunc(h.ListUserAPIKeys)))
	mux.Handle("POST /api/user/api-keys", requireAuth(http.HandlerFunc(h.UserGenerateAPIKey)))
	mux.Handle("DELETE /api/user/api-keys/{keyId}", requireAuth(http.HandlerFunc(h.UserDeleteAPIKey)))
	mux.Handle("PATCH /api/user/api-keys/{keyId}", requireAuth(http.HandlerFunc(h.UserUpdateAPIKey)))
